...
shannon_entropy = function(df,genome_size){
  ...
  df = df %>% group_by(sample, CHROM, SegmentSize) %>% mutate(chrom_shannon = sum(shannon_ntpos)) %>% dplyr::ungroup()

  df = df %>% group_by(sample) %>% mutate(genome_shannon = sum(chrom_shannon)) %>% dplyr::ungroup()
