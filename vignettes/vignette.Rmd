...
## Vignette data set
The data used in this vignette are simulated influenza genomes with specified variants at random frequencies. The data comprises 12 sampes that were sequenced twice resulting in 24 VCF files that have been annotated using *SNPeff*. 
...
This plot also makes clear that all variants were detected in both replicates. 
...
The `shannon_entropy()` function modifies the dataframe to add five new columns contaiining these values. The user must provide the variant dataframe and the length of the genome.  
```{r}
