...
## Vignette data set
The data used in this vignette are simulated influenza genomes with known mutations generated at random frequencies. These VCF files have also been annotated using SNPeff before running "Vivaldi". 
...
Metadata used for the calculations includes a .csv file containing the lengths of each of the viral segments. For non-segmented viruses, simply report the whole length of the genome. Users should also set a variable with the total length of their viral genome. 
```{r}
...
If relevant, the user should also supply a .csv file containing information about sequencing replicates containing 3 columns: the name of each VCF file, which replicate the sample represents, and the new sample name for the merged replicates. 
```{r}
...
If working with a segmented genome, users should also give a vector containing the segments in the desired order for plots. 
```{r}
...
## Merge Replicates
Sequencing in replicate improves the accuracy of variant calling, as we can require a variant to be found in both runs, as any sequencing or PCR errors are unlikely to be found in independent experiments. 
...
* the name of replicates 1 and 2 from the replicates dataframe (ex. "Rep1", "r1", "rep1", etc), which must be provided in quotes
* a vector containing the names of the columns that should be used to merge the two dataframes

 Note: This vector should contain any columns that are expected to be identical between the two replicates, such as the segment and nucleotide position of the variant. It should not contain replicate-specific information such as the allele frequency of the variant. 
```{r}
cols = c("sample","CHROM","POS","REF","ALT","ANN","ALT_TYPE","major","minor")

DF_reps = merge_replicates(VCF_DF,replicates,"rep1","rep2",cols)
```
Each row of this dataframe contains the information from a variant found in both replicates. If a variant is only found in one replicate, it will not be contained in this dataframe.

### Compare the allele frequencies between replicate 1 & 2
```{r}
ggplot2::ggplot(DF_reps, ggplot2::aes(x = minorfreq.x, y = minorfreq.y)) + ggplot2::geom_point()
```
...
```
This function calculates both the average allele frequency, as well as the weighted average allele frequency, for the major and minor allele. The average is calculated by simply taking the mean of the frequency from both replicates. The weighted average is calculated by adding the number of reads containing either the major or minor nucleotide from both replicates and dividing by the total number of reads. The average and the weighted average should be similar unless one of the replicates has significantly higher coverage per sample than the other.
